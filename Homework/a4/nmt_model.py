...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(self.model_embeddings.embed_size,
                               self.hidden_size,
                               batch_first=True,
                               bias=True,
                               bidirectional=True)
        self.decoder = nn.LSTM(self.model_embeddings.embed_size + self.hidden_size,
                               self.hidden_size,
                               batch_first=True,
                               bias=True,
                               bidirectional=False)
        ...
        self.target_vocab_projection = nn.Linear(self.hidden_size,
                                                 self.model_embeddings.target.weight.size(0),
                                                 bias=False)
    ...
    def encode(self, source_padded: torch.Tensor,
        ...
        X = source_padded.transpose(0, 1)
        batch_size = X.size(0)
        X = self.model_embeddings.source(X)
        ...
        last_cell = last_cell.transpose(0, 1).contiguous()
        last_hidden = self.h_projection(last_hidden.view(batch_size, -1))
        last_cell = self.c_projection(last_cell.view(batch_size, -1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = target_padded.transpose(0, 1).contiguous()
    ...
    def step(self, Ybar_t: torch.Tensor, dec_state: Tuple[torch.Tensor, torch.Tensor], enc_hiddens: torch.Tensor,
             enc_hiddens_proj: torch.Tensor, enc_masks: torch.Tensor) -> Tuple[Tuple, torch.Tensor, torch.Tensor]:
        ...
        _, (dec_hidden, dec_cell) = self.decoder(Ybar_t.unsqueeze(1),
                                                 (dec_state[0].unsqueeze(0), dec_state[1].unsqueeze(0)))
        dec_state = (dec_hidden.squeeze(0), dec_cell.squeeze(0))
        e_t = torch.matmul(dec_hidden.squeeze(0).unsqueeze(1), enc_hiddens_proj.transpose(1, 2)).squeeze(1)
        ### END YOUR CODE
        ...
        a_t = torch.matmul(alpha_t.unsqueeze(1), enc_hiddens)
        U_t = torch.cat([a_t, dec_hidden.transpose(0, 1)], dim=-1).squeeze(1)
