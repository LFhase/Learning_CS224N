...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(self.model_embeddings_source.e_word,
                               self.hidden_size,
                               batch_first=True,
                               bias=True,
                               bidirectional=True)
        self.decoder = nn.LSTM(self.model_embeddings_source.e_word + self.hidden_size,
                               self.hidden_size,
                               batch_first=True,
                               bias=True,
                               bidirectional=False)
    ...
    def encode(self, source_padded: torch.Tensor,
        ...
        batch_size = X.size(0)
        X = self.model_embeddings_source(X)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(Y)
        tot_steps = Y.size(1)
