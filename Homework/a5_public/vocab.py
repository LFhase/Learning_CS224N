...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            sent_char_ids = []
            for word in sent:
                sent_char_ids.append([self.start_of_word] + [self.char2id[ch] for ch in word] + [self.end_of_word])
            char_ids.append(sent_char_ids)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_char_ids = pad_sents_char(char_ids, self.char2id['<pad>'])
        char_ids_tensor = torch.tensor(padded_char_ids, device=device)
        char_ids_tensor = char_ids_tensor.transpose(0, 1).contiguous()
        ### END YOUR CODE
        return char_ids_tensor
    ...
    def from_corpus(corpus, size, freq_cutoff=2):
        ...
        print('number of word types: {}, number of word types w/ frequency >= {}: {}'.format(
